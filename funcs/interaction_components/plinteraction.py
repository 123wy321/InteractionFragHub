...
class Ligand(Mol):
    def __init__(self, mol, mol_water):
        ...
        self.hbond_acc_atoms = self.find_hba()   #添加配体原子的名称
        self.num_rings = len(self.rings)

        self.num_rot_bonds = Descriptors.NumRotatableBonds(mol)

        self.hbond_don_atom_pairs = self.find_hbd()  #添加配体原子的名称
...
class PLInteraction:
    ...
    def __init__(self, lig_obj, bs_obj, pdbid):
        ...
        self.all_hbonds_ldon = hbonds(self.protein.get_hba(),self.ligand.get_hbd(), False, 'strong')
        self.all_hbonds_pdon = hbonds(self.ligand.get_hba(),self.protein.get_hbd(), True, 'strong')
