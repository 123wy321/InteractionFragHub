...
from funcs.interaction_components.utils import vecangle, vector, euclidean3d, projection, is_sidechain
from funcs.interaction_components.utils import whichresnumber, whichrestype, whichchain, get_atom_coords
...
def hydrophobic_interactions(atom_set_a, atom_set_b):
    ...
    data = namedtuple(
        ...
        'distance restype resnr reschain restype_l resnr_l reschain_l pro_res_name lig_atom_name')
    pairings = []
    for a, b in itertools.product(atom_set_a, atom_set_b):
        ...
        contact = data(
            ...
            pro_res_name=restype+str(resnr),
            lig_atom_name=b.atom.GetProp('atomLabel')
        )
...
def hbonds(acceptors, donor_pairs, protisdon, typ):
    ...
    data = namedtuple(
        ...
        'restype reschain resnr_l restype_l reschain_l sidechain atype dtype pro_res_name lig_atom_name')
    pairings = []
    for acc, don in itertools.product(acceptors, donor_pairs):
        ...
        if not config.HBOND_DIST_MIN <= dist_ad <= config.HBOND_DIST_MAX:
            continue
        vec1, vec2 = vector(
        ...
        contact = data(
            ...
            pro_res_name=restype+str(resnr),
            lig_atom_name=ligatom.GetProp('atomLabel')
        )
        pairings.append(contact)
    return filter_contacts(pairings)
